...
?>
	...
	global $wpdb;
					...
					$result = (array_filter($output, function ($var) { return ($var[field_1]>= date('Y-m-d', strtotime($yday. ' -1 days'))); }));
					
$prayer_dwpb=dwpb_get_option('prayer_dwpb')-1;
					$current = reset($result);
					$upcoming=next($result);
					$LOL = false;
					if($upcoming[fajr]){
					if($current[field_1] <= date('Y-m-d', strtotime($yday. ' + '.$prayer_dwpb.' days')))
					{$prayer_str = "In sha Allah on <strong>". date('D, M j', strtotime($current[field_1]. ' + 1 days')) ."</strong>";
						
						if($upcoming[fajr] != $current[fajr])
						{
						$prayer_str=$prayer_str ." Fajr will be at <strong>$upcoming[fajr]</strong> " ;
						$LOL =true;
						}
						if($upcoming[zuhr] != $current[zuhr])
						{
						$prayer_str=$prayer_str ." Zuhr will be at <strong>$upcoming[zuhr]</strong>" ;
						$LOL =true;
						}
						if($upcoming[asr] != $current[asr])
						{
						$prayer_str=$prayer_str ." Asr will be at <strong>$upcoming[asr]</strong>" ;
						$LOL =true;
						}
						if($upcoming[magrib] != $current[magrib])
						{
						$prayer_str=$prayer_str ." Magrib will be at <strong>$upcoming[magrib]</strong>" ;
						$LOL =true;
						}
						if($upcoming[isha] != $current[isha])
						{
						$prayer_str=$prayer_str ." Isha will be at <strong>$upcoming[isha]</strong>" ;
						$LOL =true;
						}
						
						
					}}
		update_option(dwpb_bar_text, $prayer_str?: "As-salam-mu-alikum! This is a sample message.");	
		if(empty($prayer_str))
		update_option(dwpb_enable, "no");
		else
		update_option(dwpb_enable, "yes");
